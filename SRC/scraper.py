	...
	*/*;q=0.8",
	...
	while pag < 30:
		for proxy in shuffled_proxies: # vamos variando el proxy
			...
			responde = True
			elapsed = 2 #tiempo de espera por defecto, si el servidor no responde este crece de forma exponencial para permitir que se recupere.

			while responde and proxi_cons_request < 2 and pag < 30:# mientras el proxy responda y le hallamos hecho menos de 2 peticiones
				try:
					time.sleep(elapsed+randint(0, 2)) #espera por defecto de 2 segundos + espera aleatoria entre 0 y 2 segundos.
					...
					web = http.get( "https://www.amazon.es/s?k=smartphone&i=electronics&page="+str(pag_ini+pag)+"&__mk_es_ES=%C3%85M%C3%85%C5%BD%C3%95%C3%91&qid=1603579092&ref=sr_pg_3"+str(pag_ini+pag) , headers = headers, proxies = {'http':proxy,'https': proxy}, timeout=5 )
					...
					#ha ido todo bien, reducimos el tiempo de espera por defecto
					elapsed=sqrt(elapsed)
				except:
					...
					# ha ocurrido un error, aumentamos de forma exponencial el tiempo de espera base
					elapsed = elapsed**2
		...
		for proxy in shuffled_proxies:
			...
			while responde and proxi_cons_request < 1 and enlace < 444 : # en este caso solo quiero una petición cada vez (ya que la imagen realiza otra)
				...
				try:
					...
					web = http.get( "https://"+enlaces[enlace_ini+enlace], headers = headers, proxies = {'http':proxy,'https': proxy}, timeout=5 )
					print("OK")
					soup = BeautifulSoup(web.content,"lxml")
					enlace = enlace + 1
					proxi_cons_request = proxi_cons_request + 1
					...
					try:
						time.sleep(elapsed+randint(0, 3))#espera por defecto de 2 segundos + espera aleatoria entre 0 y 3 segundos.
					...
					#########################################################################################################
					# el servidor ha respondido, se ha recuperado. Reducimos el tiempo de espera por defecto:
					elapsed=sqrt(elapsed)

				except Exception as e:
					responde = False
					# el servidor no responde, aumentamos de forma exponencial el tiempo de espera base
					elapsed=elapsed**2
